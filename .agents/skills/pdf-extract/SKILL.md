...
1 bài:
```
python .Codex/skills/pdf-extract/extract.py searched_papers/Layer_X/<paper_id> --force
```
Toàn bộ (mỗi bài 1 subprocess → reset RAM, chống OOM tích lũy):
```
python .Codex/skills/pdf-extract/run_all.py            # chạy hết, ghi extraction_summary.json
python .Codex/skills/pdf-extract/run_all.py --min-skip 95   # bỏ qua bài đã đạt ≥95
python .Codex/skills/pdf-extract/run_all.py --only tasin    # chỉ bài khớp tên
```
...
```
python .Codex/skills/pdf-extract/recompute_qa.py      # re-probe + re-clean + re-score
```
