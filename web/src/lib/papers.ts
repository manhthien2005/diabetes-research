...
export { ROLE_META, ROLE_KEYS, ROLE_RANK } from './roles-public';
export type { PaperRole, PaperStatus } from './roles-public';
...
export interface PaperMeta {
  ...
  /** Vai trò trong ĐỀ TÀI ĐÃ CHỐT (QA_LOG Q007) — nguồn: metadata.role */
  role: PaperRole | null;
...
export interface PaperSummary {
  ...
  /** verdict trước lần chạy lại 26/07/2026 (giữ lại để đối chiếu, không mất dữ liệu) */
  verdict_prev?: string;
  verdict_reason_prev?: string;
  verdict_checked_at?: string;
...
function asRole(v: unknown): PaperRole | null {
  return typeof v === 'string' && (ROLE_KEYS as string[]).includes(v)
    ? (v as PaperRole)
    : null;
}

function asStatus(v: unknown): PaperStatus {
  return v === 'analyzed' || v === 'chosen' || v === 'rejected' ? v : 'searched';
}
...
function readPaperFolder(
  ...
  return {
    ...
    reject_reason: asString(meta.reject_reason),
    // role ghi ở metadata.json (nguồn chính); summary.json chỉ là bản sao
    role: asRole(meta.role) ?? asRole((summary as { role?: unknown })?.role),
    role_note:
      asString(meta.role_note) ??
      asString((summary as { role_note?: unknown })?.role_note),
  };
}
...
export function libraryStats() {
  const all = listAllPapers();
  const byRole = {} as Record<PaperRole, number>;
  for (const r of ROLE_KEYS) byRole[r] = 0;
  for (const p of all) if (p.role) byRole[p.role] += 1;
  return {
    total: all.length,
    ...
    byRole,
    mustRead: byRole.design + byRole.method + byRole.positioning,
