...
import { NCKH_ROOT, IMPLEMENT_DIR } from './paths';
import {
  STATUSES,
  type ProgressData,
  type Task,
...
function findProgressFile(): string {
  const defaultPaper = path.join(
    IMPLEMENT_DIR,
    'Paper_01_NHANES_NoLab',
    'PROGRESS.json'
  );
  if (fs.existsSync(defaultPaper)) return defaultPaper;

  try {
    if (fs.existsSync(IMPLEMENT_DIR)) {
      const subs = fs.readdirSync(IMPLEMENT_DIR, { withFileTypes: true });
      for (const sub of subs) {
        if (sub.isDirectory()) {
          const candidate = path.join(IMPLEMENT_DIR, sub.name, 'PROGRESS.json');
  ...
  return path.join(IMPLEMENT_DIR, 'PROGRESS.json');
}

export const PROGRESS_FILE = findProgressFile();
