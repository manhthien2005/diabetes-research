...
export function Quiz({ onJumpToChapter }: QuizProps) {
  const [persist, setPersist] = useLocalStorage<QuizStats>('explorex.kb.quiz', EMPTY_STATS);

  // bộ câu hỏi hiện tại (mặc định = toàn bộ, xáo sau mount)
  const [order, setOrder] = useState<QuizQuestion[]>(QUIZ);
  const [idx, setIdx] = useState(0);
  const [picked, setPicked] = useState<number | null>(null);
  const [results, setResults] = useState<Record<string, boolean>>({});

  // xáo trộn sau khi mount để tránh mismatch SSR (Math.random không chạy lúc render tĩnh)
  useEffect(() => {
  ...
  // khi vừa hoàn thành bộ, lưu điểm
  useEffect(() => {
    if (done && total > 0) {
  ...
  return (
    <div className="quiz" style={{ ['--ch' as string]: ch?.hue ?? 'var(--accent)' } as React.CSSProperties}>
      ...
      <div className="quiz-card">
        {ch && (
          <button className="quiz-chip" onClick={() => onJumpToChapter(q.chapter)} type="button">
        ...
        {picked !== null && (
          <div className={`quiz-feedback${picked === q.answer ? ' ok' : ' no'}`}>
            ...
            <div className="quiz-actions">
              <button className="quiz-link" onClick={() => onJumpToChapter(q.chapter)} type="button">
  ...
  return (
    <div className="quiz-summary">
      <div className="quiz-score-card">
        ...
        <div className="quiz-score-pct">{pct}% · {grade}</div>
        {lastScore && (
