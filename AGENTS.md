# AGENTS.md — Quy ước cho mọi AI agent làm việc trong `D:\NCKH`r
r
...
├── AGENTS.md                       ← file nàyr
│r
├── .claude\                        ← TOÀN BỘ skill + template của agentr
│   ├── skills\r
│   │   ├── paper-finder\           tìm bài báo mới, gán Layerr
│   │   ├── paper-analyzer\         phân tích PDF → analysis.html (tiếng Việt)r
│   │   ├── paper-comparator\       so sánh paper trong cùng Layerr
│   │   └── pdf-extract\            utility: PDF → extracted.mdr
│   ├── templates\r
...
│r
├── webapp\                         ← Research Hub (Next.js local) — GUI đọc/ghi kho paperr
│   │                                 KHÔNG phải nguồn chân lý; chỉ là giao diện trên filesystem.r
│   ├── src\                          mã nguồn (KHÔNG đụng khi làm nghiên cứu)r
│   └── data\hub.db                   SQLite: CHỈ thống kê nguồn search + settings webr
│r
└── searched_papers\                ← TẤT CẢ paper agent tìm đượcr
    ├── Layer_1_Pipeline_Nen_Tang\r
    │   └── <paper_id>\r
    │       ├── source.pdf           file gốcr
    │       ├── metadata.json        thông tin chuẩn (xem §5)r
    │       ├── extracted.md         text extract từ PDF (pdf-extract sinh)r
    │       ├── analysis.html        phân tích tiếng Việt (paper-analyzer sinh)r
    │       ├── comparison.md        so sánh trong layer (paper-comparator sinh)r
    │       ├── notes.md             ghi chú của user (webapp tab "Ghi chú" — đọc/sửa được)r
    │       └── highlights.json      vùng tô PDF của user (webapp — overlay, KHÔNG sửa source.pdf)r
    ├── Layer_2_Model_Hieu_Qua\r
    ├── Layer_3_Dataset_EHR\r
    └── Layer_4_XAI_Trien_Khai\r
```r
r
> **`notes.md` / `highlights.json`** do Research Hub (webapp) sinh ra từ thao tácr
...
---r
r
## 4. Quan hệ giữa `chosed_papers/` và `searched_papers/`r
r
- `searched_papers/` là **superset** của `chosed_papers/`. Mọi paper agent tìm được đều vào đây trước.r
- `chosed_papers/` chỉ chứa các PDF mà **user đã duyệt** sau khi đọc analysis.r
- Agent **KHÔNG** được tự ý move/copy paper sang `chosed_papers/`. Chỉ user mới có quyền promote.r
- Khi user yêu cầu "nâng cấp / mở rộng / so sánh / cải tiến" — agent PHẢI dùng `chosed_papers/` làm baseline, không phải `searched_papers/`.r
r
---r
r
## 5. Schema `metadata.json` (cho mọi paper trong `searched_papers/`)r
r
...
- `status` = **vòng đời paper**. LƯU Ý: webapp suy ra "đã chọn" từ **sự hiện diện folder trong `chosed_papers/`**, KHÔNG từ field `status` → `status:"chosen"` chỉ là ghi chú phụ.r
...
File mẫu đã render (xem trước layout): `searched_papers/Layer_1_Pipeline_Nen_Tang/gr2024_random_oversampling_diabetes/analysis.html`.r
r
...
r
Trước khi tạo folder trong `searched_papers/`, paper PHẢI thỏa cả 3 tiêu chí dưới (nếu fail bất kỳ → reject, báo user lý do):r
r
...
r
- **Không drift sang topic ngoài scope** (xem §1). Bài/topic KHÔNG phải diabetes prediction → từ chối/reject (ghi `rejected.json` nếu đã có folder). retinopathy / CGM / image → từ chối, gợi ý quay về diabetes prediction tabular/EHR.r
...
- **Không tạo file mới ngoài 3 folder gốc** (`.claude/`, `chosed_papers/`, `searched_papers/`). Nếu cần file mới → hỏi user.r
...
r
Agent PHẢI:r
1. Đọc TẤT CẢ paper trong `chosed_papers/Layer_<n>/` của layer liên quan trước.r
2. Đọc `analysis.html` / `overview.md` của các paper đó trong `searched_papers/`.r
...
1. Quét `searched_papers/Layer_*/*/metadata.json`, lấy paper cór
...
- Khi reject 1 paper đã có folder trong `searched_papers/`: thêm vào `rejected.json`r
...
- §7 vẫn là tiêu chí gốc để nhận/loại. §11 chỉ thêm: nơi ghi lý do loạir
  (`rejected.json`) + cơ chế hàng đợi (`analysis_status`).r
...
- **`extracted.md`**: bản trích PDF **trung thực, đầy đủ, giữ cấu trúc** (bảng→markdown,r
  ...
  Re-trích 1 bài: `python .claude/skills/pdf-extract/extract.py <paper_dir> --force`;r
  toàn bộ: `python .claude/skills/pdf-extract/run_all.py [--min-skip 95]`;r
...
- **`RESEARCH_BRIEF.md`** (root): bản tóm tắt trạng thái nghiên cứu, sinh từ webr
...
- **`search_pool.json`** (root): ExploreX ghi đè sau MỖI lần search — pool kết quảr
  ...
  phán giữ/loại từng bài theo §7 + scope §1. Bài loại → ghi vào `rejected.json`r
...
- **`RESEARCH_LOOP.html`** (root): sơ đồ vòng lặp 8 bước + ai làm gì. Mở để nhớ luồng.r
r
### Vòng lặp chuẩn (xem RESEARCH_LOOP.html)r
...
r
File **`QA_LOG.md`** (root) + bản máy đọc **`qa_log.json`** lưu các câu hỏi **chiến lược /r
định hướng / phương pháp luận** của user và câu trả lời đã chốt. Mục đích: agent các phiên SAUr
học **cách user nghĩ + nguyên tắc đã chốt**, không trả lời lại từ đầu. Khác `RESEARCH_BRIEF.md`r
(trạng thái paper) và `DECISION_BOARD.md` (gợi ý promote) — đây là *tư duy định hướng*.r
r
- **ĐỌC `QA_LOG.md` đầu mỗi phiên** (cùng `RESEARCH_BRIEF.md`) để nắm định hướng + ưu tiên của user.r
- Sau MỖI câu hỏi định hướng của user + câu trả lời đã chốt → **append 1 entry** vào CẢ HAI filer
  (`QA_LOG.md` thêm section + 1 dòng mục lục; `qa_log.json` push vào `entries`). **Append-only**,r
...
File **`PROGRESS.json`** (root) là **nguồn chân lý về "đang ở đâu, làm gì tiếp"**.r
r
- **ĐỌC đầu mỗi phiên**, cùng `START_HERE.md` + `QA_LOG.md`. Đây là thứ trả lời câu hỏir
  user hay hỏi nhất: *"tuần này làm gì?"*r
- **Giao diện**: trang `/tien-do` của ExploreX (`webapp/src/app/tien-do/`). Webapp chỉ là GUI —r
...
`START_HERE.md` (ngắn, chống ngợp) → `PROGRESS.json` (làm gì tiếp) →r
`CHEATSHEET.md` (tra cứu thay cho đọc paper) → `TO_DO.md` (bản đồ đầy đủ) → `QA_LOG.md` (vì sao quyết vậy).r
...
Thư mục `02_Implementation/<Paper_XX>/qc/` được phép tạo. Mọi file QA/audit do skill sinh (design_audit.md, rob_audit.json, checklist output...) ghi vào đây — không ghi vào `searched_papers/` hay `chosed_papers/`. Xem mẫu: `02_Implementation/Paper_01_NHANES_NoLab/qc/`.
